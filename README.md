...
手法は `TraCI/v1/`（ベースライン）と `TraCI/v2/`（EDF統一調停・新規）に分かれています。  
   ...
   - v2手法（EDF統一調停・新規）： `uv run python -m v2 <seed> <inflow> <mlc_ratio> [--env NAME]`  
     例：`uv run python -m v2 1 3400 0.5`（総流入3400 veh/h・必須LC比率0.5・既定env=diverge）。`--env diverge`/`--env merge` で評価環境を切替（現状 net があるのは diverge①・merge② の2つ）。

> 設定ファイルは手法ごとに分離： `config/v1/`（ベースライン用 high-way）／`config/v2/`（v2用 diverge①・merge②）。
