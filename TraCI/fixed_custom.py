...
def run(inflow_pass, inflow_exit):
    ...
    tail_position_csv = f"simulationStatistics/statistics/fixed/tail_positions_pass{inflow_pass}_exit{inflow_exit}_seed{seed}.csv"
...
def _add_vehicle():
    ...
    if sumo_time in departTime_r_pass:
        ...
        instance = FixedCustomCAV(veh_id)
...
# タイムスペース図のプロット
def _plot_time_space_diagram(output_dir="simulationStatistics/statistics/fixed"):
    if seed != "2":
        return
    ...
    for lane, data in lane_data.items():
        ...
        output_path = os.path.join(
            output_dir,
            f"fixed_{inflow_pass}_{inflow_exit}_{lane}_time_space_diagram.png",
        )
...
def _create_file_name():
    return f"fixed_pass{inflow_pass}_exit{inflow_exit}_seed{seed}"
...
if __name__ == "__main__":
    ...
    stats = SimulationStatistics(
        filename=filename, output_dir="simulationStatistics/statistics/fixed"
    )
