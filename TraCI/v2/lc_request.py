...
def active_request(o: VehObs, sim_time: float, mainlane_edge: str) -> LCRequest | None:
    """観測値から活性な必須LC要求を構成する。窓外・目標到達済み・必須LCなしなら None。"""
    if not in_activation_window(mainlane_edge, o.road, o.target_lane, o.deadline_pos, o.lane, o.lane_pos):
        return None
    ...
    return LCRequest(o.veh_id, direction, o.lane, o.target_lane, o.deadline_pos, k, o.lane_pos, wait_time)
...
def build_requests(snap: Snapshot) -> list[LCRequest]:
    ...
    for o in snap.obs.values():
        req = active_request(o, snap.sim_time, snap.mainlane_edge)
        if req is not None:
            requests.append(req)
    return requests
