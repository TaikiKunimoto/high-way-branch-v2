...
def is_insertion_safe(req: LCRequest, req_speed: float, snap: Snapshot) -> bool:
    ...
    """
    for vid in snap.lane_members.get(f"{snap.mainlane_edge}_{next_lane(req)}", []):
        o = snap.obs[vid]
        ...
        if o.lane_pos < req.current_pos:
            ...
            if speed_diff > 0:
                required += g_req(o.speed) * (speed_diff / MAX_SPEED)
        else:
            ...
            required = VEH_LENGTH + MIN_GAP * 1.5
            if speed_diff > 0:
                required += g_req(req_speed) * (speed_diff / MAX_SPEED)
        if gap < required:
            return False
    return True
