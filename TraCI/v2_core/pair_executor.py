...
def execute_pairs(
    ...
    for a in assignments:
        ...
        if req is None or requester is None or provider is None:
            continue
        target_lane = next_lane(req)
        if is_insertion_safe(req, requester.params.speed, snap) and _slot_free(
            committed, target_lane, req.current_pos
        ):
            traci.vehicle.changeLane(requester.params.id, target_lane, 0)
            committed.setdefault(target_lane, []).append(req.current_pos)
            lc_count += 1
        else:
            _provider_yield(provider, requester)
    return lc_count
...
def _provider_yield(provider: V2CAV, requester: V2CAV) -> None:
    ...
    if speed_diff > 0:
        required += g_req(p.speed) * (speed_diff / MAX_SPEED)
    target_speed = _supporting_speed(r.speed, current_gap, required)
